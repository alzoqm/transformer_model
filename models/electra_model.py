...
class GeneratorEncoder(tf.keras.Model):
  def __init__(self, max_len, seg_type, vocab_size, num_layers, dff, d_model, emb_size, num_heads, dropout):
    ...
    # self.pos_emb = tf.keras.layers.Embedding(max_len + 1, emb_size) #Discriminator와 동일한 임베딩을 사용
...
class ElectraPretrain(tf.keras.Model):
  def __init__(self, config, **kwargs):
    ...
    self.dis_linear = tf.keras.layers.Dense(2)

  def gen_loss_function(self, labels_lm, labels_cls, logits_lm, logits_cls, inputs):
    mask = tf.math.not_equal(labels_lm, -1)  #-1이 아닐 경우 True
    ...
    loss_lm = tf.multiply(loss_lm, mask_float)  #labels_lm이 -1인 mask의 값은 0이다. 따라서 labels_lm이 값이 -1인 loss는 0으로 만들어준다.
  ...
  def call(self, inputs, segments, labels_cls, labels_lm):
    gen_outputs, gen_outputs_cls, gen_attn_mat_list = self.generator(inputs, segments, self.word_emb, self.seg_emb, self.pos_emb)
    ...
    gen_loss, mask, labels_lm_custom = self.gen_loss_function(labels_lm, labels_cls, gen_outputs, gen_outputs_cls, inputs)
    ##sampling
    #labels_lm_custom -> 모든 문장이 다 들어가있음
    gen_outputs_softmax = tf.nn.softmax(gen_outputs, axis=2)
    sampling = []
    for one_batch in gen_outputs_softmax:
      """
      softmax 확률에 따른 sampling을 진행, 2차원 텐서로 입력을 받기 때문에,
       for문을 통해 batch size만큼 반복(더 효과적인 방법 찾아봐야 할 듯)
      """
      one_batch_sampling = tf.random.categorical(one_batch, 1, dtype=tf.int32) 
    ...
    return gen_loss, sampling, dis_loss
...
if __name__ == '__main__':
  ...
  gen_loss, sampling, dis_loss = electra_pretrain(x, seg, label_cls, x)
